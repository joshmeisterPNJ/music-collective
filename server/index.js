...
// ── resolve paths & pick the right .env (already loaded above) ──
const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);

// Abort test runs that point at a non-test database
if (
...
// R2 presign / local-disk uploads
app.use('/api/uploads', uploadsRouter);

// Static access to local-disk uploads (fallback/dev)
...
app.post("/api/auth/register", async (req, res) => {
  ...
  // 2. only allow superadmin after the first account
  ...
  if (adminCount > 0) {
    ...
    // first-ever admin must be superadmin
    if (role !== "superadmin") {
      return res
        .status(400)
  ...
  // 4. grant “members” permission to plain admins
  if (role === "admin") {
    const perm = await pool.query(
      "SELECT id FROM permissions WHERE key = $1",
      ["members"]
  ...
  try {
    await pool.query(
      `INSERT INTO members
         (id, name, role, email, admin_id, join_date)
       VALUES ($1, $2, 'Artist', $3, $1, COALESCE($4, CURRENT_DATE))
       ON CONFLICT (admin_id) DO NOTHING;`,
      [
        newAdminId,                       // $1 = admin.id AND member.id
        username,                         // $2 = display name
        `${username}@placeholder.local`,  // $3 = dummy email
        joinDate                          // $4 = optional join_date
      ]
    );

    // advance the sequence so we never collide
    await pool.query(
      `SELECT setval('members_id_seq',
                     GREATEST((SELECT MAX(id) FROM members), 1));`
    );
  } catch (err) {
...
app.use((err, req, res, next) => {
  // express.json throws err.type==='entity.too.large'
  // multer throws err.code==='LIMIT_FILE_SIZE'
